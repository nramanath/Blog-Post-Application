...
<body>
	...
	${post["body"]}
	<p>
		<em>Filed Under</em>: <#if post["tags"]??> <#list post["tags"] as tag>
		<a href="/tag/${tag}">${tag}</a> </#list> </#if>
	<p>Comments:
	<ul>
		<#if post["comments"]??> <#assign numComments = post["comments"]?size>
		<#else> <#assign numComments = 0> </#if> <#if (numComments > 0)>
		<#list 0 .. (numComments -1) as i>
		<form action="/like" method="POST">
			<input type="hidden" name="permalink" , value="${post["permalink"]}">
			<input type="hidden" name="comment_ordinal" , value="${i}">
			Author: ${post["comments"][i]["author"]}<br> Likes:
			${post["comments"][i]["num_likes"]} <input type="submit" value="Like">
		</form>
